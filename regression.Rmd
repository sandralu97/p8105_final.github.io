...
The visualization of the predicted lead of Biden by the case model:
...
model_case = 
  model_case %>% 
  mutate(
    lead_predicted = prediction
  )

ggplot(model_case, aes(x = mean_case, y = lead_predicted)) +
  geom_line() +
  xlab("mean number of case during polling period") +
  ylab("predicted lead of Biden over Trump")
...
The visualization of the predicted lead of Biden by the death model:
...
ggplot(model_death, aes(x = mean_death, y = lead_predicted)) +
  geom_line() +
  xlab("mean number of COVID-19 death during polling period") +
  ylab("predicted lead of Biden over Trump")
```
...
resid_1 = plot(density(resid(gee_case, type='response')))
resid_2 = plot(density(resid(gee_death, type='response')))
...
The residuals of both gee models are evenly distributed around the expected Y. However, we noticed that when the 

```{r}
predict_1 = scatter.smooth(predict(gee_case, type='response'), resid(gee_case, type='response'))
predict_2 = scatter.smooth(predict(gee_death, type='response'), resid(gee_death, type='response'))
