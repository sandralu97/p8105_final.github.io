...
scatter.smooth(predict(gee_case, type='response'), resid(gee_case, type='response'))
...
model_case %>% 
  nest(data= - state) %>% 
  mutate(
    models = map(data, ~lm(biden_lead ~ mean_case, data= .x,)),
    models = map(models,broom::tidy)
  ) %>% 
  select(-data) %>% 
  unnest(models) %>% knitr::kable(digits = 5)
