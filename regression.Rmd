...
The 17 Battleground states include Florida, Pennsylvania, Wisconsin,North Carolina, Michigan, Ohio, Minnesota, Iowa, Arizonam, Nevada, Texas, Georgia, Virginia, New Hampshirem Maine, Colorado, and New Mexico. 
...
plot(density(resid(gee_case, type='response')))

library(gee)
gee_case = gee(biden ~ mean_case, data = model_case, id = id, corstr = "exchangeable")

scatter.smooth(predict(gee_case, type='response'), rstandard(gee_case, type='deviance'))

library(statmod)

qqnorm(qresid(gee_case)); qqline(qresid(gee_case))

plot(cooks.distance(gee_case), type="response")
...
1. As COVID-19 cases increased, the difference of supporting rate between Biden and Trump decreased within each battleground state. The decrease was small but statistically significant.

2. As COVID-19 death increased, the difference of supporting rate between Biden and Trump decreased within each battle ground state. The decrease was small but statistically significant.
